...
DefFun* make_DefFun_op0(BaseType* baseType, Id* id, ParamListVoid* paramListVoid, Block* block){
  ...
  printf("DefFun* make_DefFun_op0(BaseType* baseType, Id* id, ParamListVoid* paramListVoid, Block* block)\n");
  return NULL;
}
DeclFun* make_DeclFun_op0( BaseType* baseType, Id* id, ParamListVoid* paramListVoid){
  ...
  no->u.op0.id = id;
  no->u.op0.paramListVoid = paramListVoid;
  printf("DeclFun* make_DeclFun_op0( BaseType* baseType, Id* id, ParamListVoid* paramListVoid)\n");
  return NULL;
}
DeclVar* make_DeclVar_op0(BaseType* baseType, Id* id,  Num* num0,  Num* num1){
  ...
  no->u.op0.num0 = num0;
  no->u.op0.num1 = num1;
  printf("DeclVar* make_DeclVar_op0(BaseType* baseType, Id* id,  Num* num0,  Num* num1)\n");
  return NULL;
}
...
IdArr* make_IdArr_op0(Id* id,  NumId* numId){
  ...
  no->u.op0.id = id;
  printf("IdArr* make_IdArr_op0(Id* id,  NumId* numId)\n");
  return NULL;
}
IdArr* make_IdArr_op1(Id* id){
  IdArr* no = malloc(sizeof(IdArr)); no->tag = 1;
  no->u.op1.id = id;
  printf("IdArr* make_IdArr_op1(Id* id)\n");
  return NULL;
}
...
SimpleAttr* make_SimpleAttr_op0(Id* id, Expr* expr){
  ...
  no->u.op0.id = id;
  no->u.op0.expr = expr;
  printf("SimpleAttr* make_SimpleAttr_op0(Id* id, Expr* expr)\n");
  return NULL;
}
IndexAttr* make_IndexAttr_op0(Id* id,  NumId* numId, Expr* expr){
  ...
  printf("IndexAttr* make_IndexAttr_op0(Id* id,  NumId* numId, Expr* expr)\n");
  return NULL;
}
MatAttr* make_MatAttr_op0(Id* id,  NumListList* numListList){
  ...
  printf("MatAttr* make_MatAttr_op0(Id* id,  NumListList* numListList)\n");
  return NULL;
}
MatAttr* make_MatAttr_op1(Id* id,  NumId* numId,  NumList* numList){
  ...
  printf("MatAttr* make_MatAttr_op1(Id* id,  NumId* numId,  NumList* numList)\n");
  return NULL;
}
MatAttr* make_MatAttr_op2(Id* id,  NumId* numId0,  NumId* numId1, Expr* expr){
  ...
  no->u.op2.expr = expr;
  printf("MatAttr* make_MatAttr_op2(Id* id,  NumId* numId0,  NumId* numId1, Expr* expr)\n");
  return NULL;
}
...
NumList* make_NumList_op2(Id* id){
  ...
  no->u.op2.id = id;
  printf("NumList* make_NumList_op2(Id* id)\n");
  return NULL;
}
...
Stmt* make_Stmt_op1(Id* id0, Id* id1){
  ...
  no->u.op1.id0 = id0;
  no->u.op1.id1 = id1;
  printf("Stmt* make_Stmt_op1(Id* id, Id* id1)\n");
  return NULL;
}
Stmt* make_Stmt_op2( Id* id,  NumId* numId0,  NumId* numId1){
  ...
  printf("Stmt* make_Stmt_op2( Id* id,  NumId* numId0,  NumId* numId1)\n");
  return NULL;
}
Stmt* make_Stmt_op3( Id* id,  NumId* numId){
  ...
  printf("Stmt* make_Stmt_op3( Id* id,  NumId* numId)\n");
  return NULL;
}
Stmt* make_Stmt_op4( Id* id){
  Stmt* no = malloc(sizeof(Stmt)); no->tag = 4;
  no->u.op4.id = id;
  printf("Stmt* make_Stmt_op4( Id* id)\n");
  return NULL;
}
...
Param* make_Param_op0(BaseType* baseType, Id* id){
  ...
  no->u.op0.baseType = baseType;
  no->u.op0.id = id;
  printf("Param* make_Param_op0(BaseType* baseType, Id* id)\n");
  return NULL;
}
Param* make_Param_op1(BaseType* baseType, Id* id){
  ...
  no->u.op1.baseType = baseType;
  no->u.op1.id = id;
  printf("Param* make_Param_op1(BaseType* baseType, Id* id)\n");
  return NULL;
}
...
Aux* make_Aux_op0(Id* id,  Expr* expr0,  Expr* expr1){
  ...
  printf("Aux* make_Aux_op0(Id* id,  Expr* expr0,  Expr* expr1)\n");
  return NULL;
}
Aux* make_Aux_op1(Id* id,  Expr* expr){
  ...
  no->u.op1.id = id;
  no->u.op1.expr = expr;
  printf("Aux* make_Aux_op1(Id* id,  Expr* expr)\n");
  return NULL;
}
...
NumId* make_NumId_op1(Id* id){
  ...
  no->u.op1.id = id;
  printf("NumId* make_NumId_op1(Id* id)\n");
  return NULL;
}
Call* make_Call_op0(Id* id, ArgList* argList){
  ...
  printf("Call* make_Call_op0(Id* id, ArgList* argList)\n");
  return NULL;
}
Call* make_Call_op1(Id* id){
  Call* no = malloc(sizeof(Call)); no->tag = 1;
  no->u.op1.id = id;
  printf("Call* make_Call_op1(Id* id)\n");
  return NULL;
} 
...
MatArg* make_MatArg_op0(Id* id, int int0, int int1){
  ...
  no->u.op0.int0 = int0;
  no->u.op0.int1 = int1;
  printf("MatArg* make_MatArg_op0(Id* id, int int0, int int1)\n");
  return no;
}

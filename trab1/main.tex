...
\begin{resumoumacoluna}
 Neste documento encontra-se a especificação da gramática de uma linguagem que estende uma versão mínima do C (conhecida por \textit{C-minus}). A nova linguagem proposta apresenta como primitivo o tipo matriz, bem como a realização das operações aritmécas básicas e outras operações mais complexas tais como escalonamento e potenciação.
 \vspace{\onelineskip}
...
%\setlength{\grammarparsep}{100pt plus 1pt minus 1pt} % increase separation between rules
...
%\end{grammar}
	...
	<declaration-list> ::= <declaration-list> <declaration> \alt <declaration> 
	
	<declaration> ::= <var-declaration> \alt <fun-declaration>

	<var-declaration> ::= <type-specifier> ID ; \alt <type-specifier> ID [<expression>]; \alt mat(<type-specifier>) ID [<expression>][<expression>] ;
	
	<type-specifier> ::= int \alt float
	
	<fun-declaration> ::= <type-specifier> ID ( <params> ) <compound-stmt> 
	
	<params> ::= <param-list> \alt $\epsilon$ 
	
	<param-list> ::= <param-list> , <param> \alt <param> 
	
	<param> ::= <type-specifier> ID \alt <type-specifier> ID [] \alt mat<<type-specifier-base>> ID 
	
	<compound-stmt> ::= { <local-declarations> <statement-list> } 
	
	<local-declarations> ::= <local-declarations> <var-declarations> \alt $\epsilon$ 
	
	<statement-list> ::= <statement-list> <statement> \alt $\epsilon$
	
	<statement> ::= <expression-stmt> \alt <compound-stmt> \alt <selection-stmt> \alt <iteration-stmt> \alt <return-stmt> 
	
	<expression-stmt> ::= <expression> ; \alt ; 

	<selection-stmt> ::= if ( <expression> ) <statement> \alt if ( <expression> ) <statement> else <statement> 
	
	<iteration-stmt> ::= while ( <expression> ) <statement> 
	
	<return-stmt> ::= return ; \alt return <expression> ;
	
	<expression> ::= <var> = <expression> \alt <simple-expression>
	
	<var> ::= ID \alt ID [ <expression> ]
	
	<simple-expression> ::= <additive-expression> <relop> <additive-expression> \alt <additive-expression>
	
	<relop> ::= <= \alt < \alt > \alt >= \alt == \alt !=
		
	<additive-expression> ::= <additive-expression> <addop> <term> \alt <term>
	...
	<factor> ::= ( <expression> ) \alt <var> \alt <call> \alt NUM
	
	<call> ::= ID ( <args> ) 
	
	<args> ::= <arg-list> \alt <empty> 
	
	<arg-list> ::= <arg-list> , <expression> \alt <expression> 
\end{grammar}
%%%%%%%%%%%%%%%%%%%%%%%%%%
